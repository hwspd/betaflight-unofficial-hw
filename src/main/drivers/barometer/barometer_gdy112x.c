...
#define GDY112X_I2C_ADDR_SA0_LOW    0x3A
#define GDY112X_I2C_ADDR_SA0_HIGH   0x3B
...
// Match the DPS310 driver configuration: 32 Hz output data rate and x16
// oversampling for both pressure and temperature. GDY1121 datasheet V1.1
// specifies ODR_SEL=0x0F for 32 Hz and OSR_P/OSR_T=0b100 for x16.
#define GDY112X_ODR_32HZ            0x0F

// Pressure IIR coefficient 7 is a flight-oriented compromise between noise
// reduction and response time. Temperature filtering is left disabled.
#define GDY112X_FILTER_OFF          0
#define GDY112X_FILTER_COEFFICIENT_7 3
...
static bool deviceConfigure(const extDevice_t *dev)
{
    ...
    // Configure pressure and temperature oversampling to x16, matching DPS310.
    registerWrite(dev, GDY112X_REG_OSR, (GDY112X_OSR_x16 << 4) | GDY112X_OSR_x16);

    // Configure output data rate to 32Hz, matching DPS310 (31.25ms period).
    registerWrite(dev, GDY112X_REG_ODR, GDY112X_ODR_32HZ);

    // Filter pressure with coefficient 7 while leaving temperature unfiltered.
    registerWrite(dev, GDY112X_REG_FILTER,
        (GDY112X_FILTER_OFF << 4) | GDY112X_FILTER_COEFFICIENT_7);
...
bool baroGDY112XDetect(baroDev_t *baro)
{
    ...
    const bool addressWasUnspecified = (dev->bus->busType == BUS_TYPE_I2C)
        && (dev->busType_u.i2c.address == 0);
    bool detected = false;

    deviceInit(&baro->dev, OWNER_BARO_CS);

    if (addressWasUnspecified) {
        static const uint8_t i2cAddresses[] = {
            GDY112X_I2C_ADDR_SA0_LOW,
            GDY112X_I2C_ADDR_SA0_HIGH,
        };

        for (unsigned i = 0; i < ARRAYLEN(i2cAddresses); i++) {
            dev->busType_u.i2c.address = i2cAddresses[i];
            if (deviceDetect(dev)) {
                detected = true;
                break;
            }
    ...
    // Match the DPS310 driver's Betaflight polling interval. The GDY1121
    // continues measuring internally at 32Hz while Betaflight reads at ~20Hz.
    baro->ut_delay = 0;
    baro->up_delay = 45000;
